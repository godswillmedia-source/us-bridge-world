...
exports.PublicSquareRoom = void 0;
...
class PublicSquareRoom extends colyseus_1.Room {
    constructor() {
        ...
        this.agentDeskMap = new Map();
        this.deskCounter = 0;
        // Track active sessions by cert_id
        this.activeSessions = new Map(); // cert_id → session status
    ...
    async onCreate(options) {
        ...
        this.grid = new core_1.Grid(40, 40, 16);
        // Mark furniture/walls as unwalkable (matches Game.ts layout)
        // Meeting room walls (tiles 2-14, 2-12)
        for (let x = 2; x <= 14; x++) {
            this.grid.setCollision(x, 2, true);
            this.grid.setCollision(x, 12, true);
        }
        for (let y = 2; y <= 12; y++) {
            this.grid.setCollision(2, y, true);
            this.grid.setCollision(14, y, true);
        }
        // Meeting room door gap (bottom-right)
        this.grid.setCollision(12, 12, false);
        this.grid.setCollision(13, 12, false);
        ...
        for (let y = 2; y <= 12; y++) {
            this.grid.setCollision(18, y, true);
            this.grid.setCollision(30, y, true);
        }
        // Collab door gap
        this.grid.setCollision(18, 10, false);
        this.grid.setCollision(18, 11, false);
        // Desks (approximate tile positions)
        for (let x = 4; x <= 7; x++) {
            this.grid.setCollision(x, 15, true);
            this.grid.setCollision(x, 20, true);
            this.grid.setCollision(x, 25, true);
        }
        // Coffee area (tiles 22-33, 22-33)
        for (let x = 22; x <= 33; x++) {
            this.grid.setCollision(x, 22, true);
            this.grid.setCollision(x, 33, true);
        }
        for (let y = 22; y <= 33; y++) {
            this.grid.setCollision(22, y, true);
            this.grid.setCollision(33, y, true);
        }
        // Coffee door gap
        this.grid.setCollision(22, 26, false);
        this.grid.setCollision(22, 27, false);
        ...
        this.bridge = new SupabaseBridge_1.SupabaseBridge(supabaseUrl, supabaseKey, {
    ...
    triggerDMInteraction(fromCert, targetCert) {
    ...
    updateAgentVisualState(certId, status) {
        const agentState = this.state.agents.get(certId);
        if (!agentState)
            return;
        if (status === 'active') {
    ...
    update(delta) {
        ...
        this.tickCount++;
        if (this.tickCount < 5)
            return; // Move every 5th tick (500ms)
        this.tickCount = 0;
        ...
        this.state.agents.forEach((agent, certId) => {
            ...
            if (isTalking)
                return; // Don't move while speech bubble is showing
    ...
    // --- Pathfinding ---
    navigateTo(certId, agent, targetX, targetY) {
...
//# sourceMappingURL=PublicSquareRoom.js.map